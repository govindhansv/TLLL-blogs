    ...
    <div className=" bg-white rounded-md px-6 py-10  mx-auto">
     ...
     <img className="w-screen h-60 object-cover rounded" src={img} alt="" />  
        <div>
          ...
          <input type="text"  onChange={((e) => { settitle(e.target.value) })} value={title} placeholder="Title.." id="title" className=" w-full my-2 outline-none py-1  px-2 text-md border-2 rounded-md" />
        </div>
        <div>
          <label for="name" className="text-lx block font-medium text-slate-700">Image Link</label>
          <input type="text"  onChange={((e) => { setimg(e.target.value) })}  placeholder="Paste image link.." id="name" className="w-full my-2 outline-none py-1 px-2 text-md border-2 rounded-md" />
        </div>
        <div>
          <label for="description" className="block mb-2 text-lx block font-medium text-slate-700">Blog</label>
          <textarea id="description"  onChange={((e) => { setblog(e.target.value) })} value={blog} cols="30" rows="10" placeholder="write blog here.." className="w-full font-medium  p-4 text-gray-600  outline-none rounded-md"></textarea>
        </div>
